package com.wenjun.recsys.canal;
...
public class CanalScheduling implements Runnable, ApplicationContextAware {
    ...
    private void indexES(Map<String,Object> dataMap, String database, String table) throws IOException {
        if (!StringUtils.equals("recsys",database)) {
            return;
        }
        List<Map<String,Object>> result;
        if (StringUtils.equals("seller",table)) {
            result = shopModelMapper.buildESQuery(Integer.valueOf(dataMap.get("id").toString()),null,null);
        } else if (StringUtils.equals("category",table)) {
            result = shopModelMapper.buildESQuery(null,Integer.valueOf(dataMap.get("id").toString()),null);
        } else if (StringUtils.equals("shop",table)) {
            result = shopModelMapper.buildESQuery(null,null,Integer.valueOf(dataMap.get("id").toString()));
        } else {
            return;
        }

        for (Map<String,Object> map : result) {
            //shop为Elasticsearch中构建的索引名称
